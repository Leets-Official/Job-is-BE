...
public interface JobRepository extends JpaRepository<Job, Long> {
    ...
    long countApplicable(@Param("status") JobStatus status, @Param("now") LocalDateTime now);

    // 지원 가능한 공고 중 최신순 N건 — 추천엔진 붙기 전 임시 후보 소싱용
    @Query("SELECT j FROM Job j WHERE j.status = :status "
            + "AND (j.deadlineAt IS NULL OR j.deadlineAt > :now) "
            + "ORDER BY j.postedAt DESC")
    List<Job> findApplicableJobs(@Param("status") JobStatus status, @Param("now") LocalDateTime now, Pageable pageable);
